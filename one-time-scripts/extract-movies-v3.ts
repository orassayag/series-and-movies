...
needToBeOrganizedRaw.forEach((line) => {
  ...
  quotedMovies.forEach((match) => {
    ...
    if (
      ...
      movie.length > 2 &&
      !/[?!]+$/.test(movie) &&
      !movie.includes('should definitely')
    ) {
  ...
  if (
    cleanedLine.match(/^[A-Z][A-Za-z0-9\s':&-]+$/) &&
...
candidateMovies.forEach((movie) => {
  if (
    !isMovieInList(movie, toSeeMoviesSet) &&
    !isMovieInList(movie, seenMoviesSet)
  ) {
...
const uniqueMovies = Array.from(
  new Set(moviesToAdd.map((m) => normalizeMovieName(m)))
)
  .map((normalized) => {
